...
void Solver::solve() {
    ...
    auto dx = [&](int d) { return d == EAST ? 1 : (d == WEST ? -1 : 0); };
    auto dy = [&](int d) { return d == NORTH ? 1 : (d == SOUTH ? -1 : 0); };
    auto opposite = [&](int d) { return (d + 2) % 4; };
    auto dir_char = [&](int d) -> char {
        static const char t[4] = {'n', 'e', 's', 'w'};
        return t[d];
    };
    auto dir_letter = [&](int d) -> char {
        static const char t[4] = {'N', 'E', 'S', 'W'};
        return t[d];
    };
    // member access ->
    ...
    auto set_wall = [&](int x, int y, int d, bool is_wall) {
    ...
    // Blue route preview
    auto compute_blue_route = [&](int sx, int sy) {
    ...
    auto face = [&](int target_dir, int h) {
